...
const newTabsReplace = `className={cn("flex-1 whitespace-nowrap py-2 text-[11px] sm:text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 sm:gap-2",`;
// need to replace globally
inbox = inbox.replace(new RegExp(oldTabsTarget.replace(/[.*+?^\${}()|[\\]\\]/g, '\\\\$&'), 'g'), newTabsReplace);
...
const aiModuleReplace = `                  className="flex flex-col sm:flex-row items-center sm:items-start gap-3 p-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/20 transition-all text-center sm:text-right h-full justify-center sm:justify-start"\n                >\n                  <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg flex-shrink-0", item.color)}>\n                    <item.icon className="w-6 h-6" />\n                  </div>\n                  <span className="font-bold text-white text-sm sm:mt-1">{item.title}</span>`;
...
const compDevReplace = `                  className={cn("group text-right p-4 rounded-xl border bg-slate-900/50 transition-all flex flex-col gap-3 shadow-lg h-full justify-between", item.bd)}`;
yd = yd.replace(compDevTarget, compDevReplace);

fs.writeFileSync(ydFp, yd, 'utf8');

console.log('UI Fixes Applied: Chatbot Theme, Inbox Layout, Grid Uniformity.');
